...
class AppStore extends Component {
  ...
  selectedTabItem = tabIdVal => {
    this.setState({activeTabId: tabIdVal})
  }

  getFilteredApps = () => {
    const {activeTabId} = this.state
    const filteredApps = appsList.filter(each => activeTabId === each.category)
    return filteredApps
  }
          ...
          <ul className="tabs-container">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                eachTabItem={eachTab}
                key={eachTab.tabId}
                isSelected={activeTabId === eachTab.tabId}
              />
            ))}
          </ul>
